...
//Array.prototype.map()
/*
Занятие 11 - 16-я минута (перебор массива объектов)
На 18-й минуте изменение параметров объекта в массиве ()
На 24-й обновление данных одного объекта из массива объектов с помощью map
...
const users = [
  ...
  //map своими руками (то что у него под капотом) (на 1:04 занятия 11 уточнение
  //чем отличается написание map на prototype(там this вместо array))
  /*
  - создает и возвращает новый массив
  - перебирает оригинальный массив
  - вызывает колбэк для каждого эл-та и кидает туда аргументы
  - записывает результат вызова ф-ции в новый массив
  ...
  const doubledNumbers = map(numbers, index, array) {
    console.log(number);
    console.log(index);
    console.log(array);
  ...
  //Array.prototype.filter() 27-я минута занаятие 11
  /*
...
const users = [
  ...
  // Array.prototype.find()(конец 36-й минуты занятие 11)
  /*
...
const fruits = [
  ...
  // Array.prototype.reduce()Занятие 12 начало
  //.reduce своими руками на 28-й минуте занятие 12

  /*
  Аккумулирующая функция, используется для последовательной обработки каждого элемента массива
   с сохранением промежуточного результата. Швейцарский нож функциональных методов массива.
    Возможно немного сложна в усвоении, но результат стоит того.
  Перебирает
Не изменяет
...
Легко представить ее работу на примере подсчета суммы значений массива. 
Если в качестве callback-функции задать сложение, 
то reduce вернет сумму всех значений массива. */
...
то добавит значение к аккумулятору. Когда следующего элемента не окажется (конец массива), 
функция вернет значения аккумулятора.
Вторым аргументом reduce может быть начальное значение initialValue. 
Это значение будет присвоено аккумулятору до применения callback-функции 
к первому элементу массива.
*/
const numbers = [1, 2, 3, 4, 5];
...
const tweets = [
  ...
  // Array.prototype.sort() (33-я минута занятие 12)
  /*
...
/*
Для указания своего порядка сортировки в метод arr.sort(fn) нужно передать функцию fn 
с двумя параметрами, которая сравнивает их. Внутренний алгоритм функции сортировки 
умеет сортировать любые массивы. Но для этого ему нужно знать, как их сравнивать. 
Эту роль и выполняет fn.

Алгоритм сортировки, встроенный в JavaScript, будет передавать ей для сравнения 
