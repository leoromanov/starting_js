...
Поиск DOM-узлов (14-я минута Занятие 13)
...
Свойство HTMLElement.style (26:30 - занятие 13)
...
Свойство Element.classList (36-я минута - добавление/удаление классов элемента)
...
38 минутя Занятие 13 - СОЗДАНИЕ ДОКУМЕНТА С НУЛЯ
41 минута - карточка объекта (создание независимых узлов)
46 минута - помещение узлов друг в друга (appendChild(element) and insertBefore(element))
59 минута - создание функции для добавления продуктов
...
на 7-8 минуте занятия 14 инфо почему removeEventListener может не работать
...
Событие submit (15-я минута занятие 14: куда вешать слушателя на форме)
И КАК ОТМЕНИТЬ ПЕРЕЗАГРУЗКУ СТРАНИЦЫ ПРИ ОТПРАВКЕ ФОРМЫ
...
30-я минутя занятие 14 - formData
...
в конце 30-й минуты лайфхак по решению задачи
где нужно взять значение из одного инпута, и второе из другого инпута
суммировать и записать в третий инпут

38-я минута - enabled/disabled кнопки в зависимости от чекбокса
...
События клавиатуры 
47-я минутя занятие 14
Есть три основных события клавиатуры: keydown, keypress и keyup. 
При нажатии клавиши сначала происходит keydown, после чего keypress, и только потом keyup, 
когда клавишу отжали.

События keydown и keyup срабатывают при нажатии любой клавиши, включая служебные. 
А вот keypress срабатывает только если нажата символьная клавиша, 
т. е. нажатие приводит к появлению символа. Управляющие клавиши, такие как Ctrl, Shift, Alt и другие, 
не генерируют событие keypress.




//События мыши (59-я минута занятие 14) или как делать ХОВЕР через JS



//Создание модального окна (1:11 минута)
